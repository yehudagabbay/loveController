...
export default function PerfectDateFlow({ navigation, route }) {
  ...
  const currentSelectionLabel = useMemo(() => {
    ...
    if (questionStep === 'gender') {
      return genderOptions.find((item) => item.id === gender)?.title || '';
    }
  ...
  }, [boundaryOptions, boundaries, gender, genderOptions, goal, goals, location, questionStep, t, vibe, vibes]);

  const hasCurrentSelection = questionStep !== 'gender' || Boolean(gender);
  ...
  }, [questionMotion, questionStep, questionSwipeX]);
  ...
  const loadPerfectDateDeck = async () => {
    ...
    try {
      ...
      setDeckError(
        error?.message ||
          t('perfectDate.errors.deckFailed'),
      );
      return false;
    } finally {
  ...
  const saveQuestionnaire = async () => {
    ...
    try {
      ...
      setQuestionError(
        error?.message ||
          t('perfectDate.errors.saveFailed'),
      );
    } finally {
  ...
  const questionPanResponder = useMemo(
    ...
    [goQuestionBack, goQuestionNext, hasCurrentSelection, questionSwipeX, setupLoading, t],
  );
      ...
      return (
        <View style={styles.choiceStack}>
          {genderOptions.map((item) => (
            <ToggleChip key={item.id} icon={item.icon} label={item.title} active={gender === item.id} onPress={() => setGender(item.id)} />
          ))}
        </View>
      );
    }
