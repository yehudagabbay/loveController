...
export default function GameModeSelect({ navigation, route }) {
  ...
  const handleChooseMode = (mode) => {
    ...
    if (mode === 'perfect_date') {
      Alert.alert(
        t('gameModeSelect.alerts.premiumSoonTitle', { defaultValue: 'בקרוב למנויי Premium' }),
        t('gameModeSelect.alerts.perfectDateSoonMessage', {
          defaultValue: 'הדייט המושלם יהיה זמין בקרוב כחוויה מיוחדת למנויי Premium.',
        })
      );
      return;
    }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.listViewport}>
          <FlatList
            ...
            onMomentumScrollEnd={(event) => resetWheelIfNeeded(event.nativeEvent.contentOffset.y)}
            onScrollEndDrag={(event) => resetWheelIfNeeded(event.nativeEvent.contentOffset.y)}
