...
const CATEGORY_THEME = {
  1: { name: 'Family Intro', bgColor: '#DCFCE7', icon: iconFamily },
  2: { name: 'Family Fun', bgColor: '#FEF3C7', icon: iconFamily },
  3: { name: 'Family Bond', bgColor: '#E0E7FF', icon: iconFamily },
};
...
export default function FamilyCardsGame({ route, navigation }) {
  ...
  // תרגומים לקטגוריה/רמה (מבוסס JSON)
  const categoryName = (id) =>
    t(`familyCardsGame.category.name.${String(id)}`);

  const levelName = (id) =>
    t(`familyCardsGame.level.name.${String(id)}`);
    ...
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>
          {t('familyCardsGame.emptyPlayers.message')}
        </Text>
        ...
        >
          <Text style={styles.backButtonText}>
            {t('familyCardsGame.emptyPlayers.backButton')}
          </Text>
        </TouchableOpacity>
      </View>
    );
  ...
  const handleDrawPress = () => {
    if (remainingCount === 0) {
      showInlineAlert(t('familyCardsGame.inlineAlerts.noMoreCards'), 'info');
      return;
    }
  ...
  const skipCard = () => {
    ...
    advanceTurn();
    setCurrentPlayerIndex((prev) => prev); 
    setCurrentCardId(null);
    closeCardModal();
  };
  ...
  const openFeedbackModal = () => {
    ...
    if (!userId || userId <= 0) {
      showInlineAlert(t('familyCardsGame.inlineAlerts.missingUserForFeedback'), 'error');
      return;
    }
    setFeedbackText('');
  ...
  const submitFeedback = async () => {
    ...
    const userId = getCurrentUserId(route);
    if (!userId || userId <= 0) {
      showInlineAlert(t('familyCardsGame.inlineAlerts.missingUser'), 'error');
      return;
    }
    if (!feedbackText.trim()) {
      showInlineAlert(t('familyCardsGame.inlineAlerts.feedbackMissing'), 'info');
      return;
    }
    ...
    try {
      ...
      showInlineAlert(t('familyCardsGame.inlineAlerts.feedbackSent'), 'info');
      closeFeedbackModal();
    } catch (err) {
      showInlineAlert(t('familyCardsGame.inlineAlerts.feedbackError'), 'error');
    } finally {
  ...
  return (
    <View style={styles.screen} key={lang}>
      ...
      {inlineAlertVisible && (
        <View
          style={[
            styles.inlineAlert,
            inlineAlertType === 'error' ? styles.inlineAlertError : styles.inlineAlertInfo,
          ]}
        >
      ...
      <View style={styles.header}>
        <Text style={styles.gameTitle}>{t('familyCardsGame.header.title')}</Text>
        <Text style={styles.gameSubTitle}>{t('familyCardsGame.header.subtitle')}</Text>
      </View>
        ...
        <View style={styles.deckContainer}>
          {remainingCount > 0 ? (
            ...
            <View style={styles.emptyBox}>
              <Text style={styles.emptyTitle}>{t('familyCardsGame.status.deckEmptyTitle')}</Text>
              <Text style={styles.emptySub}>{t('familyCardsGame.status.deckEmptyMessage')}</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={styles.bottomHalf}>
        ...
        <View style={styles.controlsBox}>
          <View style={styles.controlsRow}>
            ...
            >
              <Text style={styles.btnText}>{t('familyCardsGame.buttons.drawCard')}</Text>
            </TouchableOpacity>
            ...
            >
              <Text style={styles.btnTextDark}>{t('familyCardsGame.buttons.selectCards')}</Text>
            </TouchableOpacity>
          </View>
        </View>

        <View style={styles.statusBox}>
          <Text style={styles.title}>{t('familyCardsGame.status.title')}</Text>

          <Text style={styles.subTitle}>
            {t('familyCardsGame.status.completed', { done: revealedCount, total: totalCount })}
          </Text>

          {/* תור השחקן */}
          <View style={styles.playerTurnBadge}>
            <Text style={styles.playerTurnLabel}>{t('familyCardsGame.status.turnNow')}</Text>
            <Text style={styles.playerTurnName}>{players[currentPlayerIndex]}</Text>
          </View>
          ...
          {lastCombo ? (
            ...
            >
              ...
              >
                {t('familyCardsGame.status.lastCardTitle')}
              </Text>
            ...
            <Text style={styles.hint}>{t('familyCardsGame.status.hint')}</Text>
          )}
        </View>
      </View>
      ...
      <GameCardModal
        ...
        turnText={t('familyCardsGame.turn.badge', { name: players[currentPlayerIndex] })}
        showCardStatusBar
        cardStatusBarProps={{
      ...
      >
        <Pressable style={styles.backdrop} onPress={closeFeedbackModal}>
          <View style={styles.feedbackModalWrapper} onStartShouldSetResponder={() => true}>
            <Text style={styles.feedbackTitle}>{t('familyCardsGame.feedbackModal.title')}</Text>
            <Text style={styles.feedbackSubtitle}>{t('familyCardsGame.feedbackModal.subtitle')}</Text>

            <ScrollView>
              <Text style={styles.feedbackHelperText}>
                {t('familyCardsGame.feedbackModal.helperText')}
              </Text>
            </ScrollView>
            ...
            <View style={{ marginTop: 10 }}>
              <TextInput
                ...
                placeholder={t('familyCardsGame.feedbackModal.placeholder')}
            ...
            <View style={styles.feedbackButtonsRow}>
              ...
              >
                <Text style={styles.feedbackBtnText}>{t('familyCardsGame.buttons.cancel')}</Text>
              </TouchableOpacity>
              ...
              >
                ...
                <Text style={styles.feedbackBtnText}>
                  {sendingFeedback
                    ? t('familyCardsGame.feedbackModal.sending')
                    : t('familyCardsGame.buttons.sendFeedback')}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  inlineAlertInfo: { backgroundColor: '#DBEAFE' },
  inlineAlertError: { backgroundColor: '#FEE2E2' },
  inlineAlertText: {
    color: '#111827',
    fontSize: 14,
    fontWeight: '600',
  ...
  emptySub: { fontSize: 15, color: '#666', marginTop: 5, textAlign: 'center' },
  ...
  lastCardTitle: { fontSize: 12, marginBottom: 4 },
  ...
  modalCategoryText: { fontSize: 22, fontWeight: '900', letterSpacing: 1 },
  ...
  feedbackHelperText: { fontSize: 14, color: '#6B7280', marginBottom: 8, textAlign: 'right' },
